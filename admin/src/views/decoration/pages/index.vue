<template>
    <div class="decoration-pages min-w-[1100px]">
        <div class="flex flex-1 h-full justify-between">
            <el-card shadow="never" class="!border-none flex scroll-view-content" :body-style="{ 'padding-right': 0 }">
                <Menu v-model="activeMenu" :menus="menus" />
            </el-card>

            <preview class="flex-1 scroll-view-content" v-model="selectWidgetIndex" @updatePageData="updatePageData"
                :pageData="getPageData" :pageMeta="getPageMeta" />
...
const updateContent = (content: any) => {
    if (menus[activeMenu.value]?.pageData && selectWidgetIndex.value !== -1) {
...
> = reactive({
    [pagesTypeEnum.HOME]: {
        ...
        pageData: generatePageData(['search', 'banner', 'nav', 'news', 'wedding-swiper'])
    },
    [pagesTypeEnum.USER]: {
        id: 2,
        type: 2,
...
const getPageData = computed(() => {
    // console.log(menus[activeMenu.value]?.pageData)
...
const setData = async () => {
    ...
    // console.log(JSON.stringify(data.pageData))
