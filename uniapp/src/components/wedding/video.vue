<template>
    <div class="section">
        <video id="myVideo" class="myVideo" style="cursor: pointer" :src="url" :danmu-list="danmuList" enable-danmu
            danmu-btn controls :poster="poster" @play="play" @ended="ended"></video>
        <div class="btn-area">
            <input class="danmuInput" placeholder="请输入临时弹幕，真实弹幕来自留言" placeholder-style="color:#bbb"
                @blur="bindInputBlur" />
            <button @tap="bindSendDanmu">点击生成临时弹幕</button>
            <image src="@/static/images/wedding/close1.png" @tap="close" />
        </div>
    </div>
</template>
