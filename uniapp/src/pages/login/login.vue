<template>
    ...
    <view class="bg-white login min-h-full flex flex-col items-center px-[40rpx] pt-[120rpx] box-border">
        <view>
            <image :src="appStore.getWebsiteConfig.shop_logo" mode="widthFix"
        ...
        <view class="w-full mt-[140rpx] pb-[60rpx]">
            <block v-if="!phoneLogin">
                ...
                <view v-if="isOpenOtherAuth && isWeixin && inWxAuth">
                    <u-button type="primary" @click="wxLogin" :customStyle="{ height: '100rpx' }" hover-class="none">
                        用户一键登录
                    </u-button>
                </view>
                <!-- #endif -->
                ...
                <view class="mt-[40rpx]">
                    <u-button @click="phoneLogin = !phoneLogin" :customStyle="{ height: '100rpx' }" hover-class="none">
                        手机号登录
                    </u-button>
                </view>
            </block>
        ...
        ">
                    <view
                        ...
                        <u-input class="flex-1" v-model="formData.account" :border="false" placeholder="输入账号" />
                    </view>
                    <view
                        class="px-[18rpx] py-[10rpx] border border-solid border-light rounded-[10rpx] flex h-[100rpx] items-center mt-[40rpx]">
                        <u-input class="flex-1" v-model="formData.password" type="password" placeholder="输入密码"
                            :border="false" />
                ...
                <!-- 验证码登录 -->
                <template v-if="formData.scene == LoginWayEnum.MOBILE &&
        ...
        ">
            ...
            <block v-if="phoneLogin">
                <view class="mt-[60rpx]">
                    <u-button type="primary" @click="handleLogin(formData.scene)" :customStyle="{
        height: '100rpx',
        opacity: DisableStyle ? '1' : '0.5'
                ...
                <view class="flex justify-between mt-[40rpx]">
                    <view>已有账号，使用
                        <span class="text-primary" @click="changeLoginWay(LoginWayEnum.ACCOUNT)" v-if="formData.scene == LoginWayEnum.MOBILE &&
        includeLoginWay(LoginWayEnum.ACCOUNT)
        ">密码登录</span>
                        <span class="text-primary" @click="changeLoginWay(LoginWayEnum.MOBILE)" v-if="formData.scene == LoginWayEnum.ACCOUNT &&
        includeLoginWay(LoginWayEnum.MOBILE)
        ">验证码登录</span>
                    </view>
        ...
        <!-- 协议弹框 -->
        <u-modal v-model="showModel" show-cancel-button :show-title="false" confirm-color="var(--color-primary)"
            @confirm="; (isCheckAgreement = true), (showModel = false)" @cancel="showModel = false">
        ...
        <!-- #ifdef MP-WEIXIN -->
        <mp-privacy-popup v-model:show="showPrivacyPopup" @confirm="handleConfirmPrivacy"></mp-privacy-popup>
